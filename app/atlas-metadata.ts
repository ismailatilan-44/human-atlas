...
export function prepareAtlas(atlas: Atlas): Atlas {
  return {
    ...atlas,
    parts: atlas.parts.map(part => NERVOUS_PARTS.has(part.id) && part.system !== 'nervous'
      ? { ...part, system: 'nervous' }
      : part),
  };
}
...
const LABELS_BY_ID = new Map(labels.entries.flatMap(entry => entry.ids.map(id => [id, entry] as const)));
...
export function anatomyLabel(id: string, fallback: string, language: AnatomyLanguage): string {
  ...
  const side = language === 'tr'
    ? entry.side === 'left' ? 'Sol' : 'Sağ'
...
export function anatomySearchTerms(id: string, fallback: string): string[] {
  ...
  if (entry) {
    terms.push(...(['tr', 'en', 'la'] as const).map(language => anatomyLabel(id, fallback, language)), ...entry.aliases);
  }
  // Permit an ASCII keyboard to find Turkish labels while retaining readable terms.
  return [...new Set(terms.flatMap(term => [term, term.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/ı/g, 'i')]))];
}
