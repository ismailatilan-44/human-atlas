...
export default function AnatomyScene({
  ...
  const host = useRef<HTMLDivElement>(null),
    ...
    cameraChanged = useRef(onCameraChange);
  ...
  useEffect(() => {
    ...
    let lastState: SceneState | null = null,
      lastRestore: CameraPose | undefined;
    ...
    anchorLabel.className = "anatomy-anchor";
    anchorLabel.hidden = true;
    ...
    const materialFor = (system: string, ghost = false) => {
      const m = new T.MeshStandardMaterial({
        ...
        roughness: 0.53,
        side: T.DoubleSide,
        transparent: ghost || system === "integumentary",
        opacity: ghost ? 0.07 : system === "integumentary" ? 0.1 : 1,
        depthWrite: !ghost && system !== "integumentary",
      });
      m.onBeforeCompile = (shader) => {
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
          ...
          "#include <clipping_planes_fragment>\n" +
            (ghost
              ? "if (partVisible < 1.5) discard;"
              : "if (partVisible < 0.5 || partVisible > 1.5) discard;"),
        );

        shader.fragmentShader = shader.fragmentShader.replace(
          "#include <color_fragment>",
          "#include <color_fragment>\ndiffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.015, 0.22, 0.17), partSelected * 0.95);",
        );
      };
    ...
    const mats = new Map(SYSTEMS.map((s) => [s.id, materialFor(s.id)])),
      ghostMats = new Map(SYSTEMS.map((s) => [s.id, materialFor(s.id, true)]));
    let loaded = 0;
    const loadChunk = async (ci: number) => {
      const chunk = atlas.chunks[ci],
        compressed = !!chunk.gzip && typeof DecompressionStream !== "undefined";
      const response = await fetch(assetUrl(compressed ? chunk.gzip! : chunk.url), {
        signal: abort.signal,
      });
      ...
      groups.forEach((gs, system) => {
        ...
        scene.add(mesh);
        const ghostMesh = new T.Mesh(geometry, ghostMats.get(system as never));
    ...
    const up = (e: PointerEvent) => {
      ...
      pickers.forEach((mesh, i) => {
        ...
        if (hits[0]) {
          if (data[i * 4 + 3] > 1.5) {
            if (hits[0].distance < nearestGhost) {
              nearestGhost = hits[0].distance;
              foundGhost = i;
            }
          } else if (hits[0].distance < nearest) {
            nearest = hits[0].distance;
            found = i;
          }
        }
      });
      if (found < 0) found = foundGhost;
    ...
    const animate = () => {
      ...
      if (changed || moving || lastExtent < 0) {
        ...
        const context = new Set(s.anchor?.contextPartIds ?? []);
        const visibleParts = atlas.parts.filter(
          (p) =>
            !hidden.has(p.id) &&
            (s.isolate
              ? selection.has(p.id)
              : visible.has(p.system) || selection.has(p.id) || context.has(p.id)),
        );
        const nextLayoutKey =
        ...
        atlas.parts.forEach((p, i) => {
          ...
          const shown =
            !hidden.has(p.id) &&
            (s.isolate ? selected : visible.has(p.system) || selected || context.has(p.id));
          data.set(
            [
              dx,
              dy,
              dz,
              shown
                ? s.ghost && (selection.size > 0 || s.anchor) && !selected && !context.has(p.id)
                  ? 2
                  : 1
                : 0,
            ],
      ...
      if (isolateKey !== lastIsolate || (inspecting && moving)) {
        if (inspecting) {
          ...
          if (s.anchor) {
            const point = new T.Vector3().fromArray(s.anchor.position);
          ...
          if (!box.isEmpty()) {
            ...
            if (s.inspectorOpen) {
              if (landscape) {
                right = (inspectorRect?.left ?? w - 320) - 16;
                top = 100;
                bottom = h - 125;
              } else if (mobile) {
      ...
      if (s.restoreCamera && s.restoreCamera !== lastRestore) {
        const pose = s.restoreCamera;
        camera.clearViewOffset();
        if (
          pose.viewOffset &&
          pose.viewOffset.fullWidth === el.clientWidth &&
          pose.viewOffset.fullHeight === el.clientHeight
        ) {
          const v = pose.viewOffset;
      ...
      if (dirty) {
        ...
        if (s.anchor && amount <= 0.01) {
          ...
          anchorLabel.hidden = projected.z < -1 || projected.z > 1;
          anchorLabel.style.left = `${((projected.x + 1) * el.clientWidth) / 2}px`;
          anchorLabel.style.top = `${((1 - projected.y) * el.clientHeight) / 2}px`;
          anchorLabel.textContent = s.anchor.label;
        }
        cameraChanged.current?.({
          position: camera.position.toArray() as [number, number, number],
          target: controls.target.toArray() as [number, number, number],
          viewOffset: camera.view?.enabled ? { ...camera.view } : null,
        });
        targets = [];
        if (amount > 0.45) {
          const hasSolid = atlas.parts.some(
